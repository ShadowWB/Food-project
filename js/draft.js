...
window.addEventListener('DOMContentLoaded', () => {
    ...
    //     .then(data => data.json())
    //     .then(cardsArr => {
    //         console.log(cardsArr.menu)
    //         cardRendering(cardsArr.menu, ".menu .container")
    //     }).catch(e=>{
    //         console.log(e)
    //      })
    //
...
//
    function postDataBind(form) {
        form.addEventListener("submit", (event) => {
            ...
            if (localStorage.getItem("name") == formObj.name && localStorage.getItem("phone") == formObj.phone) {
                alert("HELLO " + localStorage.getItem("name"))
            } else {
                localStorage.setItem(("name"), formObj.name)
                localStorage.setItem("phone", formObj.phone)
                localStorage.setItem("date", formObj.data)
            }
            for (let i = 0; i < 10; i++) {
